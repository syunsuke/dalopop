...
-- 総数に関するもの
-----------------------------------
DROP VIEW IF EXISTS view_age_5year_short_groups_rate_total;
CREATE VIEW view_age_5year_short_groups_rate_total AS
WITH
    -- 95歳以上データを変換したもの
    to85plus AS (
    SELECT
        ...
        (0.1 * (age_85_89 + age_90_94 + age_95plus)/ age_total) as age_85plus_rate
    ...
    SELECT
        ...
        (0.1 * age_75_79 / age_total) as age_75_79_rate,
        (0.1 * age_80_84 / age_total) as age_80_84_rate,
        (0.1 * age_85plus / age_total) as age_85plus_rate
    FROM suikei_5sai_total_v001
    ),
    
    -- ２つのデータをUNIONする
    age_5year_groups_short_rate AS (
    SELECT * FROM  to85plus
    UNION ALL
    SELECT * FROM  just85plus
    )
-- 最終の出力
SELECT * FROM age_5year_groups_short_rate;

-----------------------------------
-- 男性に関するもの
-----------------------------------
DROP VIEW IF EXISTS view_age_5year_short_groups_rate_male;
CREATE VIEW view_age_5year_short_groups_rate_male AS
WITH
    -- 95歳以上データを変換したもの
    to85plus AS (
    SELECT
...
-- 女性に関するもの
-----------------------------------
DROP VIEW IF EXISTS view_age_5year_short_groups_rate_female;
CREATE VIEW view_age_5year_short_groups_rate_female AS
WITH
    -- 95歳以上データを変換したもの
    to85plus AS (
    SELECT
